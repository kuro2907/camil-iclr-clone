...
class CAMIL:
    def __init__(self, args):
        ...
        self.net = Model(
            inputs=[self.inputs["bag"], self.inputs["adjacency_matrix"]],
            outputs=[out, alpha, k_alpha],
        )
    ...
    def train(self, train_bags, dataset, fold, val_bags, args):
        ...
        if not os.path.exists(os.path.join(args.save_dir, fold)):
            os.makedirs(
                os.path.join(args.save_dir, fold, args.experiment_name), exist_ok=True
            )

        checkpoint_path = os.path.join(
            os.path.join(args.save_dir, fold, args.experiment_name),
        ...
        for epoch in range(args.epochs):
            ...
            for step, (x_batch_train, y_batch_train) in enumerate(train_gen):
                ...
                train_dict = train_step(
                    x_batch_train, np.expand_dims(y_batch_train, axis=0)
                )
    ...
    def predict(self, test_bags, dataset, fold, args, test_model):
        ...
        if self.subtyping:
            ...
            macc_0, mprec_0, mrecal_0, mspec_0, mF1_0, auc_0 = eval_metric(
                y_pred, y_true
            )
