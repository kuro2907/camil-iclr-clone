...
if __name__ == "__main__":
    ...
    train_bags = (
        references["train"]
        .apply(lambda x: os.path.join(args.feature_path, x + ".h5"))
        .values.tolist()
    )
    ...
    val_bags = (
        references.apply(lambda row: func_val(row.val), axis=1).dropna().values.tolist()
    )
    print("val_bags", val_bags)
    val_bags = ["h5_files/normal_124.h5", "h5_files/normal_065.h5"]
    test_bags = (
        references.apply(lambda row: func_val(row.test), axis=1)
        .dropna()
        .values.tolist()
    )
    print("test_bags", test_bags)
    ...
    if not args.test:
        ...
        test_acc, test_auc = test_net.predict(
            test_bags, fold_id, args, test_model=test_net.model
        )
